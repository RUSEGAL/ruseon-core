...
**RUSEON Core** — это open-source Edge-платформа для работы с видеоинфраструктурой, написанная на Go. Разработанная для облачных и Edge-сред, она обеспечивает zero-copy ретрансляцию RTSP в HLS, запись архива в fMP4 и закладывает фундаментальный API для систем видеоаналитики на базе ИИ.
...
* 🛡 **Cloud-Native Архитектура**: Встроенная защита от проблемы "Thundering Herd" и жесткий OOM-контроль для безопасной работы с тысячами одновременных потоков.
