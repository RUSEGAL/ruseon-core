...
При стандартизированном 8-часовом полнофункциональном стресс-тестировании на стабильность (**600 одновременных камер 30 FPS**, **1 800 активных HLS зрителей**, **240 WebRTC WHEP клиентов** и **двунаправленный gRPC ИИ-конвейер** на 12-ядерном процессоре):

| Компонент | Метрика | Фактический результат | Латентность / Примечания |
| :--- | :--- | :--- | :--- |
| **Входящий поток (Ingest)** | **18 180 FPS (1 342.9 Mbps)** | 600 камер @ 30 FPS | `0` потерянных кадров (523.6 млн кадров за 8ч) |
| **Отдача HLS fMP4** | **501.9 MB/s** (17 625 404 сегмента) | 1 800 одновременных зрителей | `p50: 3.59 ms` / `p95: 211.2 ms` (15.1 ТБ отдано) |
| **REST API Роутер** | **460 RPS** (13.2 млн запросов, 0 ошибок) | Полная JWT валидация | `p50: 0.59 ms` / `p95: 8.52 ms` |
| **WebRTC WHEP** | **2 439 152 RTP пакета** | 240 живых пир-соединений | `p50: 390.3 ms` / `p95: 935.4 ms` (0 ошибок) |
| **gRPC ИИ-конвейер** | **18 180 FPS / 1 993 RPS** | Передача кадров и метаданных | Задержка стрима: `p50: 28.45 ms` (0 ошибок) |
| **Шина событий EventBus** | **11 042 761 доставленное событие** | 100% успешная доставка | `0` дропов (0.0% потерь) |
| **Память и стабильность** | **707 MB RSS** (95 MB Heap Alloc) | 348 активных горутин | Макс. пауза GC: `7.69 ms` (ср: `1.03 ms`) |

> 💡 **Топология тестирования:** Генератор нагрузки (эмуляция 600 камер, 1 800 HLS-клиентов, 240 WebRTC-плееров, 20 gRPC ИИ-воркеров) и медиа-сервер RUSEON Core работали совместно на одном 12-ядерном хосте (in-process loopback), активно разделяя процессорные ресурсы и сетевой стек ОС на протяжении всех 8 часов теста.
